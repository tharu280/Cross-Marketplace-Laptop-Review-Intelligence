...
# Create and activate the environment (using the same name as development)
...
pip install streamlit pandas plotly requests # Ensure these are installed
...
Generate Data Artifacts (if not included or need regeneration):

Run the dynamic database setup (creates backend/laptops_dynamic.db):
...
Run the FAISS index creation (creates backend/laptops.index and backend/laptops_metadata.json):
...
(Ensure the source JSON files derived from PDFs are present in the /data directory if running the index script)
...
Navigate to the project root directory (/laptop agent).
